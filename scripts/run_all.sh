...
step "9/10 발표자료(PPTX·PDF)"
"$PY" scripts/07_deck.py
# 발표자 화면에 뜨는 것은 슬라이드 노트다. 대본에서 옮겨 붙인다.
"$PY" scripts/23_notes.py
# 대본의 시간표 세 곳은 손으로 적으면 어긋난다. 대본에서 잰다.
"$PY" scripts/21_time_script.py --write
...
# AI 가 쓴 티가 나는 표현. 내용과 무관하게 점수가 깎이는 자리다.
"$PY" scripts/22_audit_aitell.py
