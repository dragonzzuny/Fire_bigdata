...
<img src="reports/brand/logo_lockup_plain.png" alt="불씨예보 K-FIREBIRD" width="340">
...
![LightGBM](https://img.shields.io/badge/LightGBM-Poisson-0B7285)
![Streamlit](https://img.shields.io/badge/Streamlit-7%20screens-FF4B4B?logo=streamlit&logoColor=white)
![tests](https://img.shields.io/badge/tests-292%20passed-2EA44F)
![data](https://img.shields.io/badge/data-bigdata--119.kr-C92A2A)
...
<img src="docs/images/03_allocation.png" alt="가용 인력 기준 예방점검 배분 화면">

<div align="center">
<sub>점검관 4명이 20일 동안 할 수 있는 <b>640건</b>을 제약으로 넣으면, 실제로 갈 수 있는 <b>186개 구역</b>이 남습니다.<br>
위험도 상위 20%(292개 격자)를 그대로 가려면 28,818건이 필요합니다 — 가용 인력으로는 한 구역도 끝내지 못합니다.</sub>
</div>

<div align="center">

|  같은 인력으로 담는 화재  |  미학습 연도 포착률  |  무작위 배정 대비  |  저장소 자동 검사  |
|:---:|:---:|:---:|:---:|
|  0% → **17.8%**  |  상위 20%가 **68.5%**  |  **3.43배**  |  **292개 시험**  |
...
```mermaid
flowchart LR
    A["소방안전<br/>빅데이터"] --> B["500m 격자<br/>의사결정 테이블"]
    B --> C["화재위험 예측<br/>LightGBM Poisson"]
    C --> D["인력 제약 배분<br/>배낭 · 형평성"]
    D --> E["관서별 순찰 동선<br/>OSRM · TSP"]
    E --> F["공문 계획서<br/>법정 별지 서식"]
    style C fill:#fdecea,stroke:#c92a2a
    style D fill:#e6f4ea,stroke:#2f9e44
    style F fill:#e7f0fb,stroke:#1c7ed6
```
...
Streamlit 대시보드 7화면. 인원·기간·순찰 유형을 바꾸면 배분과 동선이 함께 바뀝니다.
...
<tr>
<td><img src="docs/images/08_hydrant.png" alt="소방용수 사각지대 화면"><br>
<sub><b>대응취약 구역</b> · 위험 상위인데 소화전이 없는 90개 구역, 소화전 신설 우선순위</sub></td>
<td><img src="docs/images/09_assistant.png" alt="화재예방 업무 도우미 화면"><br>
<sub><b>업무 도우미</b> · 소방 법령 7종 · 조문 457개에서 근거를 붙여 답합니다</sub></td>
</tr>
</table>

### 순찰 유형을 바꾸면 동선이 바뀝니다

<img src="docs/images/11_route_compare.png" alt="순찰 유형별 동선 비교">

<sub>같은 관서, 같은 12개 구역이라도 <b>일반 예방순찰</b>과 <b>다중이용업소 야간순찰(22–02시)</b>은
대상이 달라 동선이 달라집니다. 거리는 직선이 아니라 실제 도로 주행거리입니다.</sub>
...
<table>
<tr>
<td width="50%"><img src="docs/images/07_plan_head.png" alt="예방순찰 계획서 머리"><br>
<sub><b>일반기안문 배열</b> · 기관·수신·경유·제목·시행일·근거 조문까지 공문 서식 그대로</sub></td>
<td width="50%"><img src="docs/images/04_form_compare.png" alt="법정 별지 서식 자동 작성"><br>
<sub><b>법정 별지 서식</b> · 법제처 원본 괘선 위에 27칸 중 17칸을 값으로 채웁니다</sub></td>
</tr>
</table>

<img src="docs/images/06_plan_doc.png" alt="계획서 본문 — 근거 조문과 결재란">

<sub>순찰 구역 목록·이동거리·누적시간, 중점 확인 사항, <b>근거 조문 원문</b>, 용어 유의사항,
붙임과 결재란까지 한 장에 들어갑니다. 법정 서식의 대표자·전화번호 칸은 공개 데이터로 채울 수
있어도 비워 둡니다 — 빈칸을 그럴듯하게 메우는 순간 결재 문서가 아니라 추정치가 됩니다.</sub>
...
시험이 지킵니다(`tests_firebird/test_operations.py`). 관할 다섯 곳에 위험 비중만큼
최소 배분을 걸어도 포착률 손실은 **-0.3%p** 에 그칩니다.
...
**④ 달마다 무게가 다릅니다** — 계절과 기상(실효습도)으로 월 위험계수를 냅니다.
계절만 쓰면 설명력 0.14, 기상을 넣으면 0.29입니다.

<img src="docs/images/12_monthly_risk.png" alt="월별 화재위험 계수" width="620">
...
| 확인한 것 | 결과 |
|---|---|
| 2021년을 맞히는가 | 상위 20%가 그해 화재의 **68.5%** (95% CI 63.5~72.9%) |
| 전년 화재 순보다 나은가 | **+5.0%p** (CI +1.6~+8.0%p) |
| 무작위 대비 | **3.43배** (PAI) |
| 특정 구·군만 잘 맞는가 (LOGO) | 50.9~61.3%, **다섯 관할 전부** 단순 기준 상회 |
| 울산 밖에서도 되는가 | 세종 74.0% (표본이 작아 단순 기준 72.1%와 유의차 없음) |
| 주소가 거칠어 부풀려졌나 | 도로명 있는 건만으로도 70.8% — 부풀림 없음 |
| 그해 이전 자료만으로 짰다면 | 60구역(관내 4.1%)이 그해 화재 **35.9%** 를 담음. 3년 연속 단순 기준 대비 +38~+47건, 신뢰구간 모두 0 초과 |

</td><td width="48%">

<img src="docs/images/10_capture_curve.png" alt="2021년 화재 포착률 곡선">

<sub>가로축은 점검·순찰을 집중하는 상위 위험 격자 비율, 세로축은 그 격자에서 실제로
일어난 화재의 비율. 학습에 쓰지 않은 2021년입니다.</sub>
...
공개 데이터만 사용하며 개인정보를 다루지 않습니다(구역 집계 단위).
