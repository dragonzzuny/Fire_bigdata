...
NOT_AVAILABLE = {
    ...
    "유동인구": ("생활인구 자료 미연계", "통신사 유동인구(유상). 무료 공개 자료 없음"),
    # SGIS OpenAPI 는 읍면동까지만 준다. 격자 인구는 파일데이터로만 배포된다.
    # 읍면동 값을 이 칸에 쓰면 한 동의 인구가 그 동의 모든 격자에서 다시 세어진다.
    "상주인구": ("격자 단위 인구 미연계",
              "SGIS 격자 통계 파일데이터(공공데이터포털) — OpenAPI 는 읍면동까지만"),
