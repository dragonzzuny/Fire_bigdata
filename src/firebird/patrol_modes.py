...
class PatrolMode:
    ...
    #: 기본 순찰 격자 수
    default_k: int = 15
    #: 이 순찰과 관련된 조문. 검색으로 찾은 조문보다 이쪽을 먼저 쓴다.
    #: 어휘 검색은 '소방시설·피난' 같은 낱말만 보고 화재예방안전진단 절차 규정을
    #: 일반 순찰의 근거로 끌어오기도 한다. 근거가 어긋나면 결재 때 신뢰를 잃는다.
    #:
    #: **주의**: 이 조문들은 '순찰을 하라'는 규정이 아니다. 소방 법령 457개
    #: 조문과 별표·서식 68건을 뒤져도 '순찰'이라는 말은 한 번도 나오지 않는다.
    #: 예방순찰은 소방기본법 제3조의 '화재 예방·경계' 업무를 관서가 스스로
    #: 정한 방식으로 수행하는 것이고, 아래 조문들은 **순찰 중 발견한 위험에
    #: 무엇을 할 수 있는가**(조치·조사 권한)를 말한다. 이 구분을 흐리면
    #: 결재선에서 '순찰 근거가 제7조냐'는 질문에 답할 수 없다.
    legal_refs: tuple[str, ...] = ()
    #: 이 업무에 쓰는 법정 서식(시행규칙 별지). 우리가 만든 계획서는 내부 기안용이고,
    #: 실제 처분·기록은 법정 서식으로 해야 한다. 담당자가 찾아 헤매지 않게 짚어 준다.
    official_forms: tuple[str, ...] = ()


#: 예방순찰이라는 활동 자체의 근거.
#:
#: 소방 법령에 '순찰'은 없다. 있는 것은 소방기본법 제3조의 '화재 예방·경계'
#: 라는 업무이고, 그것을 어떤 방법으로 몇 번 할지는 법이 정하지 않는다.
#: 즉 **방법은 관서 재량**이며, 그래서 지금은 경험과 관행으로 정해진다.
#: 이 서비스가 근거를 대는 자리가 정확히 거기다.
#:
#: 반면 화재안전조사는 법정 의무다. 화재예방강화지구는 시행령 제20조 제1항이
#: '연 1회 이상'을 못박고 있다. 순찰과 조사를 같은 말로 쓰면 안 된다.
DUTY_BASIS = (
    "소방기본법 제1조",       # 화재를 예방ㆍ경계하거나 진압하고…
    "소방기본법 제3조",       # 시ㆍ도의 화재 예방ㆍ경계ㆍ진압 및 조사… 소방업무
)

#: 순찰 중 위험을 발견했을 때 쓸 수 있는 권한. 모든 유형에 공통이다.
ACTION_BASIS = (
    "화재의 예방 및 안전관리에 관한 법률 제17조",   # 화재의 예방조치 등
    "화재의 예방 및 안전관리에 관한 법률 제7조",    # 화재안전조사로 이어질 때
)
...
MODES: dict[str, PatrolMode] = {
    "general": PatrolMode(
        ...
        checks=("소화기·유도등 등 기본 소방시설 상태",
                ...
                "옥외 가연물 방치",
                "노후 전기설비·문어발 배선"),
        official_forms=("화재안전조사 조치명령서", "화재예방 조치명령서"),
    ...
    "night_business": PatrolMode(
        ...
        checks=("영업 중 비상구 잠금·폐쇄 여부 (중점)",
                ...
                "화재감지기 임의 차단 여부",
                "내부 마감재 방염성능"),
        official_forms=("화재안전조사 조치명령서", "화재안전조사 조치명령 대장"),
    ...
    "market": PatrolMode(
        ...
        checks=("점포 간 방화구획·연소확대 위험",
                ...
                "소화기·옥외소화전 접근성",
                "심야 무인 상태의 화기 관리"),
        official_forms=("화재예방강화지구 관리대장", "화재예방 조치명령서"),
    ...
    "vulnerable": PatrolMode(
        ...
        checks=("야간 근무 인력 및 대피 계획 숙지",
                ...
                "객실·병실 감지기 작동",
                "피난안내도 게시"),
        official_forms=("자위소방대 및 초기대응체계 교육ㆍ훈련 실시 결과 기록부",),
    ...
    "dry_season": PatrolMode(
        ...
        checks=("옥외 소각·화기 취급 행위",
                ...
                "건축 공사장 화기 관리",
                "쓰레기 집하장·폐기물 야적"),
        official_forms=("화재예방 안전조치 협의 신청서", "화재예방 조치명령서"),
